...
class _OrderPlacedState extends State<OrderPlaced> {
  ...
  _getSellerOrder() async {
    var sellerAllOrderList =
        await DashRepository.getAllSellerOrderPlaced().catchError((e) {
      setState(() {
        isLoading = false;
      });
    });
    ...
    return Scaffold(
        body: isLoading
            ...
            : sellerOrder.length == 0
                ? Center(child: NoDataAvailable(message: "No Placed Available"))
                : ListView.builder(
                    itemCount: sellerOrder.length,
                    itemBuilder: (BuildContext context, int index) {
                      return _buildOrder(
                        sellerOrder[index].quantity,
                        sellerOrder[index].when,
                        sellerOrder[index].amount,
                        sellerOrder[index].paid,
                        sellerOrder[index].paidAmount,
                        sellerOrder[index].method,
                        sellerOrder[index].remaining,
                        sellerOrder[index].status,
                        sellerOrder[index].productId,
