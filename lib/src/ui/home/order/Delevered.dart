...
class Delevered extends StatefulWidget {
  @override
  _DeleveredState createState() => _DeleveredState();
}
...
class _DeleveredState extends State<Delevered> {
  ...
  _getUserOrder() async {
    var userOrderList =
        await HomeReposiitory.getUserDeliveredOrder().catchError((e) {
      setState(() {
        isLoading = false;
      });
    });
            ...
            : userOrder.length == 0
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (BuildContext context, int index) {
                      return _buildOrder(
                        userOrder[index].id,
                        userOrder[index].quantity,
                        userOrder[index].when,
                        userOrder[index].amount,
                        userOrder[index].paidAmount,
                        userOrder[index].method,
                        userOrder[index].remaining,
                        userOrder[index].status,
                        userOrder[index].productId,
                      );
                    }));
  }
